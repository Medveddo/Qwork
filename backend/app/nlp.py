...
def extract_temperature(tokens: List[DocToken]) -> Optional[float]:
    temp_tokens = [
        token
        for token in tokens
        if token.lemma in RECOMENDATIONS_KEYWORDS["temperature"]
    ]
    if not temp_tokens:
        return 0
    ...
    numerics = [
        token for token in tokens if token.pos == "NUM" or isfloat(token.text)
    ]
    if not numerics:
        return 0
    ...
    numerics = [
        token
        for token in numerics
        if not any(separator in token.text for separator in separators)
        and 33.0 <= float(token.text) <= 45.0
    ]
...
def get_numeric_tokens_with_separator(tokens: List[DocToken]) -> List[DocToken]:
    return [
        token
        for token in tokens
        if any(separator in token.text for separator in separators)
    ]
...
def extract_blood_pressure(
    ...
    blood_pressure_tokens = [
        token
        for token in tokens
        if token.lemma in RECOMENDATIONS_KEYWORDS["blood_pressure"]
    ]
    ...
    numerics = [
        token
        for token in tokens
        if token.pos == "NUM" or isfloat(token.text) or has_digits(token.text)
    ]
    ...
    if with_separator:
        if len(with_separator) == 1:
            ...
            logger.warning(
                f"MORE THAN ONE NUMERICS WITH SEPARATOR: {with_separator}"
            )
    ...
    if list_of_matches := numerics_with_cconj_between(tokens):
        if len(list_of_matches) == 1:
            ...
            logger.warning(
                (
                    f"MORE THAN ONE GROUP OF NUMERICS WITH"
                    f"CCONJ BETWEEN: {list_of_matches}"
                )
            )
